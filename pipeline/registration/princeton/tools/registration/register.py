...
def elastix_registration(reg_vol, cores=5, **kwargs):  
  ...
  '''
    ...
    sys.stdout.write('Beginning registration on {}'.format(reg_vol.brainname)); sys.stdout.flush()
    ...
    if not os.path.exists(reg_vol.resampled_for_elastix_vol):
        sys.stdout.write('\n   Resizing {}'.format(reg_vol.downsized_vol)); sys.stdout.flush()
        ...
        [vol.add_registration_volume(reg_vol.resampled_for_elastix_vol) for vol in vols]
    ...
    if False and 'atlas_scale' in kwargs: 
        ...
        AtlasFile = convert_to_mhd(AtlasFile, dims=kwargs['atlas_scale'], dst = os.path.join(kwargs['outputdirectory'], os.path.splitext(os.path.basename(kwargs['AtlasFile']))[0])+'.mhd')
        ...
        reg_vol.add_resampled_for_elastix_vol(convert_to_mhd(reg_vol.resampled_for_elastix_vol, dims=resampled_zyx_dims[::-1], dst = os.path.join(kwargs['outputdirectory'], os.path.splitext(os.path.basename(reg_vol.resampled_for_elastix_vol))[0])+'.mhd'))

    #ELASTIX
    e_out_file, transformfile = elastix_command_line_call(AtlasFile, reg_vol.resampled_for_elastix_vol, svlc, parameters)    
    ...
    resample(reg_vol.downsized_vol+'.tif', AtlasFile, svlocname=bluechannel, singletifffile=True) ##needs to be resample(not resample_par) as you need EXTACT dimensions
    color_movie_merger(AtlasFile, e_out_file, svlc, reg_vol.brainname + '_bluechanneloriginal', movie5=bluechannel)
    ...
    combine_images(str(reg_vol.resampled_for_elastix_vol), AtlasFile, os.path.join(gridfld, 'result.tif'), e_out_file, svlc, reg_vol.brainname) 
    ...
    #appy transform
    [transform_function(vol, reg_vol, svlc, cores, AtlasFile, parameters, transformfile, resampled_zyx_dims) for vol in vols_to_register]           
    ...
    writer(outdr,"\n\n*************STEP 3************************\nelastix has completed using:\nbrainname: {}\nAtlasFile: {}\nparameterfolder: {}\nparameter files {}\nsvlc: {}\n****************\n".format(reg_vol.brainname, AtlasFile, reg_vol.parameterfolder, parameters, svlc))
...
def similarity_transform(fx, mv, dst, nm, level='fast', cleanup=True):
    ...
    #make dir
    makedir(dst)
    out = os.path.join(dst, 'tmp'); makedir(out)
...
def apply_transformix(vol, reg_vol, svlc, cores, AtlasFile, parameters, transformfile, resampled_zyx_dims):
    ...
    sig_ch_resized = vol.downsized_vol+'.tif'; trnsfrm_outpath=os.path.join(svlc,os.path.basename(vol.downsized_vol)); makedir(trnsfrm_outpath)
    ...
    if not vol.ch_type == 'regch': resample(sig_ch_resized, AtlasFile, svlocname=sig_ch_resampledforelastix, singletifffile=True, resamplefactor=1.3)                
    ...
    if resampled_zyx_dims: sig_ch_resampledforelastix= convert_to_mhd(vol.resampled_for_elastix_vol, dims=resampled_zyx_dims[::-1])
        
    #run transformix        
    sp.call(['transformix', '-in', sig_ch_resampledforelastix, '-out', trnsfrm_outpath, '-tp', transformfile])
...
def apply_transformix_and_register(vol, reg_vol, svlc, cores, AtlasFile, parameters, transformfile, resampled_zyx_dims):
    ...
    sig_out = os.path.join(svlc,os.path.basename(vol.downsized_vol)); makedir(sig_out)
    sig_to_reg_out=os.path.join(sig_out, 'sig_to_reg'); makedir(sig_to_reg_out)
    ...
    if not vol.ch_type == 'regch': 
        ...
        resample(sig_ch_resized, AtlasFile, svlocname=sig_ch_resampledforelastix, singletifffile=True, resamplefactor=1.3)                
        #cannot be resample_par because that be a pool inside of pool        
    ...
    [shutil.copy(os.path.join(svlc, xx), os.path.join(sig_to_reg_out, 'regtoatlas_'+xx)) for xx in os.listdir(svlc) if 'TransformParameters' in xx]
    ...
    #run transformix        
    sp.call(['transformix', '-in', sig_ch_resampledforelastix, '-out', sig_out, '-tp', reg_to_atlas_tps[-1]])
...
def change_interpolation_order(fl, order = 3):
    ...
    with open(fl, 'r') as f, open(fl1, 'w') as new:
            for line in f:
                new.write('(FinalBSplineInterpolationOrder "{}")\n'.format(order)) if 'FinalBSplineInterpolationOrder' in line else new.write(line)
...
def change_bspline_interpolation_order(fl, order = 3):
    ...
    with open(fl, 'r') as f, open(fl1, 'w') as new:
            for line in f:
                new.write('(BSplineTransformSplineOrder "{}")\n'.format(order)) if 'BSplineTransformSplineOrder' in line else new.write(line)
...
def allen_compare(AtlasFile, svlc, trnsfrm_out_file, verbose=False, outline = False):  
    ...
    color=[os.path.join(trnsfrm_outpath, 'proj0.png'), os.path.join(trnsfrm_outpath, 'proj1.png'), os.path.join(trnsfrm_outpath, 'proj2.png')]
    nametosave=[os.path.join(trnsfrm_outpath, "proj0_overlay.png"), os.path.join(trnsfrm_outpath, "proj1_overlay.png"), os.path.join(trnsfrm_outpath, "proj2_overlay.png")]
    ###
    if outline:
        import pylab as pl
        #cntlst = []; 
    ...
    depth.layout(nametosave[0], nametosave[1], nametosave[2], trnsfrm_outpath) ###might need 1,0,2 for some files
...
def inj_and_cells(elastixfld, cellch, injch, AtlasFile, threshold = 0.075):
    ...
    ninjfld=os.path.join(elastixfld, 'injection_and_cells'); makedir(ninjfld)
    ...
    ###apply overlay
    color=[os.path.join(ninjfld, 'proj0.png'), os.path.join(ninjfld, 'proj1.png'), os.path.join(ninjfld, 'proj2.png')]
    nametosave=[os.path.join(ninjfld, "proj0_overlay.png"), os.path.join(ninjfld, "proj1_overlay.png"), os.path.join(ninjfld, "proj2_overlay.png")]
    for x in range(3):
...
def make_inverse_transform(vol_to_process, cores = 5, **kwargs):
    ...
    vol_to_process.add_resampled_for_elastix_vol(vol_to_process.downsized_vol+'_resampledforelastix.tif')

    if not os.path.exists(vol_to_process.resampled_for_elastix_vol):
        print ('Resizing')        
        resample_par(cores, vol_to_process.downsized_vol+'.tif', AtlasFile, svlocname=vol_to_process.resampled_for_elastix_vol, singletifffile=True, resamplefactor=1.3)
        print ('Past Resizing')
    
    ####setup
    parameters = []; [parameters.append(os.path.join(parameterfolder, files)) for files in os.listdir(parameterfolder) if files[0] != '.' and files [-1] != '~']; parameters.sort()       

    ###set up save locations
    svlc = os.path.join(outdr, 'elastix_inverse_transform'); makedir(svlc)
    svlc = os.path.join(svlc, '{}_{}'.format(vol_to_process.ch_type, vol_to_process.brainname)); makedir(svlc)
    ...
    #atlas(mv)->reg (fx)
    atlas2reg = os.path.join(svlc, reg_vol.resampled_for_elastix_vol[reg_vol.resampled_for_elastix_vol.rfind('/')+1:-4]+'_atlas2reg'); makedir(atlas2reg)
    e_out_file, e_transform_file = elastix_command_line_call(fx=reg_vol.resampled_for_elastix_vol, mv=AtlasFile, out=atlas2reg, parameters=parameters)        

    #reg(mv)->sig(fx)
    reg2sig = os.path.join(svlc, vol_to_process.resampled_for_elastix_vol[vol_to_process.resampled_for_elastix_vol.rfind('/')+1:-4]+'_reg2sig'); makedir(reg2sig)
    e_out_file, e_transform_file = elastix_command_line_call(fx=vol_to_process.resampled_for_elastix_vol, mv=reg_vol.resampled_for_elastix_vol, out=reg2sig, parameters=parameters)        
     
    ##set up transform series:
    atlas2reg2sig = os.path.join(svlc, vol_to_process.resampled_for_elastix_vol[vol_to_process.resampled_for_elastix_vol.rfind('/')+1:-4]+'_atlas2reg2sig'); makedir(atlas2reg2sig)
    #copy transform paramters
    [shutil.copy(os.path.join(reg2sig, xx), os.path.join(atlas2reg2sig, 'reg2sig_'+xx)) for xx in os.listdir(reg2sig) if 'TransformParameters' in xx]
    [shutil.copy(os.path.join(atlas2reg, xx), os.path.join(atlas2reg2sig, 'atlas2reg_'+xx)) for xx in os.listdir(atlas2reg) if 'TransformParameters' in xx]
    
    #connect transforms by setting regtoatlas TP0's initial transform to sig->reg transform
    tps = [os.path.join(atlas2reg2sig, xx) for xx in os.listdir(atlas2reg2sig) if 'TransformParameters' in xx]; tps.sort(reverse=True) #they are now in order recent to first, thus first is regtoatlas_TransformParameters.1.txt
...
def point_transform_due_to_resizing(array, chtype = 'injch', svlc = False, **kwargs):
    ...
    #    
    if not svlc: pnts_fld=os.path.join(outdr, 'transformedpoints_pretransformix'); makedir(pnts_fld)
    if svlc: pnts_fld=os.path.join(svlc); makedir(pnts_fld)
    ...
    sys.stdout.write('...done writing centers.\nSaved in {}'.format(pnts_fld)); sys.stdout.flush()
...
def point_transformix(txtflnm, transformfile, dst=False):
    ...
    from subprocess import check_output
    #set paths    
    if not dst: trnsfrm_outpath = os.path.join(os.path.dirname(transformfile), 'posttransformix'); makedir(trnsfrm_outpath)
    if dst: trnsfrm_outpath = os.path.join(dst, 'posttransformix'); makedir(trnsfrm_outpath)
...
def collect_points_post_transformix(src, point_or_index = 'point'):
    ...
    for i in range(len(lines)):        
        arr[i,...]=lines[i].split()[lines[i].split().index(idx)+3:lines[i].split().index(idx)+6] #x,y,z
