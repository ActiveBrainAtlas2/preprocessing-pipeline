...
if __name__ == '__main__':
    ...
    # except:
    param_fld = os.path.join(PIPELINE_ROOT, "pipeline/registration/princeton/parameterfolder") 
    atl = "/home/eddyod/.brainglobe/sagittal_atlas_20um_iso.tif" # PMA 20 micron isotropic
    print(src)
    print(reg)
    assert os.path.exists(param_fld)
    if stepid == 0:
        print("Doing normal registration")
        mv = "/net/birdstore/Active_Atlas_Data/data_root/pipeline_data/DK37/preps/CH1/downsized_for_atlas.tif"
        assert os.path.exists(mv)
        print("\nPath to downsized vol for registration to atlas: %s" % mv)
        fx = atl
        assert os.path.exists(fx)
        print("\nPath to atlas: %s" % fx)
        out = os.path.join(src, "elastix")
