...
class UrlGenerator:
    ...
    def add_segmentation_layer(self, folder_name, layer_name):
        segment_layer = dict(
            type="segmentation",
            source="precomputed://https://activebrainatlas.ucsd.edu/data/structures/"
            + folder_name,
            tab="segments",
            name=layer_name,
        )
    ...
    def add_precomputed_image_layer(self, source, name, shader=None):
        image_layer = dict(type="image", source=source, tab="source", name=name)
    ...
    def get_url(self):
        if hasattr(self, "dimensions"):
            return json.dumps(
                {
                    "dimensions": {
                        "x": [self.dimensions[0], "m"],
                        "y": [self.dimensions[1], "m"],
                        "z": [self.dimensions[2], "m"],
                    },
                    "layers": self.layers,
                }
            )
