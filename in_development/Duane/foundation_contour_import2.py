...
def load_transform(animal):
    ...
    downsampled_transforms = create_downsampled_transforms(animal, transforms, downsample=False)
...
def get_offsets(animal):
    braini = Brain(animal)
    section_size = np.array((braini.sqlController.scan_run.width, braini.sqlController.scan_run.height))
...
def main():
    ...
    for animal in animals:
        ...
        output = ""
        for structure_name, dict_section_x_y_pairs in aligner.contour_per_structure_per_section.items():
            output += f"DEBUG - [{animal}] (structure/brain region):" + str(structure_name) + "; QTY:" + str(len(dict_section_x_y_pairs)) + "\n"
            print(f"DEBUG - ANIMAL: {animal}, (structure/brain region):", structure_name, "; QTY:", len(dict_section_x_y_pairs))

            FK_structure_id = structure_data.get(structure_name)[0] #LOOKUP STRUCTURE ID
            aligned_points= []
            ...
            for sectioni, contour_points_sectioni in dict_section_x_y_pairs.items():
                ...
                if animal == 'MD594' and str(structure_name) == 'VLL_R' and (str(sectioni) == '300' or str(sectioni) == '308'):
                    continue
                if animal == 'MD594' and str(structure_name) == 'VLL_L' and str(sectioni) == '150':
                    continue
                if animal == 'MD594' and str(structure_name) == 'VCP_R' and (str(sectioni) == '332' or str(sectioni) == '336'):
                    continue
                if animal == 'MD589' and str(structure_name) == 'RtTg_S' and (str(sectioni) == '220' or str(sectioni) == '222' or str(sectioni) == '223' or str(sectioni) == '240'):
                    continue
                if animal == 'MD589' and str(structure_name) == 'Sp5I_R' and (str(sectioni) == '337'):
                    continue
                if animal == 'MD589' and str(structure_name) == 'VLL_L' and (str(sectioni) == '155'):
                    continue
                ...
                aligned_contour_points_sectioni = transform_points(np.array(contour_points_sectioni), transform_per_section[sectioni]) 
                # aligned_contour_points_sectioni = aligned_contour_points_sectioni*32

                ordering_int = 0
                segment_id = binascii.hexlify(os.urandom(20)).decode() # 20-byte (40 character) random hex string

                np_scaled_x_y_pairs = np.array(aligned_contour_points_sectioni)*.452 # APPLY SCALING FACTOR (x *.452, y * .452)
