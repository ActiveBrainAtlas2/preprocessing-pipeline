...
class NumpyToNeuroglancer():
    ...
    def __init__(self, animal: str, volume, scales, layer_type, data_type, num_channels=1,
        chunk_size = [64,64,64], offset = [0,0,0]):
        self.volume = volume
        self.scales = scales
    ...
    def add_segmentation_mesh(self, layer_path, mip = 0) -> None:
        ...
        tasks = tc.create_meshing_tasks(layer_path, mip=mip, compress=True, sharded=False, progress=False) # The first phase of creating mesh
        tq.insert(tasks)
        tq.execute()
