...
def load_transforms(
    ...
    if use_inverse:
        ...
        for fn, T_down32 in sorted(Ts_down32.items()):
            ...
            T_rescaled[:2, 2] = (
                T_down32[:2, 2] * 32.0 * planar_resolution / string_to_voxel_size
            )
...
def load_consecutive_section_transform(animal, moving_filepath, fixed_filepath):
    ...
    param_file = os.path.join(
        ELASTIX_DIR,
        moving_filepath + "_to_" + fixed_filepath,
        "TransformParameters.0.txt",
    )
    if not os.path.exists(param_file):
        raise Exception(
            "Transform file does not exist: %s to %s, %s"
            % (moving_filepath, fixed_filepath, param_file)
        )
...
def create_downsampled_transforms(animal, transforms, downsample):
    ...
    for img_name, tf in transforms.items():
        transforms_to_anchor[img_name] = convert_2d_transform_forms(
...
def dict_to_csv(d, fp):
    df = pd.DataFrame.from_dict(
        {k: np.array(v).flatten() for k, v in d.items()}, orient="index"
    )
...
def convert_2d_transform_formsXXX(transform, out_form):
    if isinstance(transform, str):
        if out_form == (2, 3):
            ...
            return np.vstack(
                [
                    np.reshape(list(map(np.float, transform.split(","))), (2, 3)),
                    [0, 0, 1],
                ]
            )
    else:
...
def convert_cropbox_fmt(
    out_fmt, data, in_fmt=None, in_resol="1um", out_resol="1um", stack=None
):
    if in_resol == out_resol:
        in_resol = "1um"
        out_resol = "1um"
    arr_xywh_1um = convert_cropbox_to_arr_xywh_1um(
        data=data, in_fmt=in_fmt, in_resol=in_resol, stack=stack
    )
    data_out = convert_cropbox_from_arr_xywh_1um(
        data=arr_xywh_1um, out_fmt=out_fmt, out_resol=out_resol, stack=stack
    )
...
def clean_image(file_key):
    """
    CURRENT scipi affine_transformation USES ~5x RAM OF ORIGINAL FILE
    """
    ...
    print(
        f"TOTAL TIME (clean_image): {round(total_elapsed_time, 1)}s, FILE INDEX: {index}"
    )
    return
