...
class cell_extractor:
    ...
    def image_filename(self,ch='CH3',tile=0):
        return "{p}/{c}/{s}/{s}tile-{t}.tif".format(p=self.path,c=ch,s=self.section,t=tile)

    def read_manual_marks(self)
        csv_path="{p}/{c}/{s}/*.csv".format(p=self.path,c='CH3',s=self.section)
