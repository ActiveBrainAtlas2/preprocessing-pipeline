...
with viewer1.txn() as s:
    s.layers.clear()
    s.layers.append(name = 'DK39', layer = fixed_layer)
    s.layers.append(name = 'DK52', layer = moving_layer)
    annotations = get_annotations(DK52_coms)
    s.layers.append(name="DK52 com",
...
with viewer2.txn() as s:
    s.layers.clear()
    s.layers.append(name = 'DK39', layer = fixed_layer)
    s.layers.append(name = 'DK52 transformed', layer = transformed_layer)
    annotations = get_annotations(prepi_itk_transformed)
    s.layers.append(name="DK52 com transformed",
                layer=neuroglancer.LocalAnnotationLayer(dimensions=dimensions,
                annotations=annotations,
                annotationColor = "#FB9F89"))
